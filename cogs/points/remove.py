...
class remove(commands.Cog):
  ...
  @app_commands.command(
    name = "remove",
    description = "Remove points from users")
  ...
  async def remove(
    ...
    embed = discord.Embed(title="Remove", description=f"{user}'s balance", color=0xff0000)
    embed.add_field(name="Removed Points", value=f"{amount}")
